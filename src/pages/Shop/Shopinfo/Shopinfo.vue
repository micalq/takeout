<template>
    <div class="shop-info" >
        ...
        <div class="activity">
        <div class="activity-item" :class="supportClass[item.type]" v-for="(item, index) in shopInfo.supports" :key="index">
...
export default {
    ...
    methods: {
        initScroll(){//初始化scroll
           ...
           if(!this.shopInfo.pics)//从别的路由组件切换过来走这里 如果没数据return
            return     
            const liWidth=120,space=6;//计算ul宽
            console.log(this.shopInfo.pics.length)
            let count=this.shopInfo.pics.length
            this.$refs.picul.style.width=count*(liWidth+space)-space+"px"
        }
    },
    watch: {
        shopInfo(){//用watch监视到当前vuex里面的shopinfo有数据了才执行后面的
            this.initScroll()
        }
    },
}
